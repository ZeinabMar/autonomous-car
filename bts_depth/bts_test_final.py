...
def test(params):
    ...
    num_test_samples = 1 #get_num_lines(args.filenames_file)
    ...
    with torch.no_grad():
        for _, sample in enumerate(tqdm(dataloader.data)):
            ...
            pred_depths = depth_est.cpu().numpy().squeeze()
            pred_8x8s = lpg8x8[0].cpu().numpy().squeeze()
            pred_4x4s = lpg4x4[0].cpu().numpy().squeeze()
            pred_2x2s = lpg2x2[0].cpu().numpy().squeeze()
            pred_1x1s = reduc1x1[0].cpu().numpy().squeeze()
    ...
    #         filename_image_png = save_name + '/rgb/' + lines[s].split()[0].split('/')[-1]
        # else:/content/workspace/bts/pytorch/result_bts_nyu_v2_pytorch_densenet161/cmap
    scene_name = 'car' # lines[s].split()[0].split('/')[0]
    ...
    rgb_path = args.data_path # os.path.join(args.data_path, './' + lines[s].split()[0])
    ...
    cmap = plt.cm.viridis
    pred_depth_scaled_1 = 255 * cmap(pred_depth_scaled)[:,:,:3]
    pred_depth_scaled = pred_depth_scaled_1.astype(np.uint8)

    print("ss",pred_depth_scaled_1.shape)
    cv2.imwrite(save_name + '/raw/' + scene_name+'1'+'.png', pred_depth_scaled_1, [cv2.IMWRITE_PNG_COMPRESSION, 0])

    pred_depth_scaled = pred_depth_scaled.astype(np.uint16)
    cv2.imwrite(filename_pred_png, pred_depth_scaled, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    cv2.imwrite(save_name + '/raw/' + scene_name+'1'+'.png', pred_depth_scaled, [cv2.IMWRITE_PNG_COMPRESSION, 0])

    if True : #args.save_lpg:
