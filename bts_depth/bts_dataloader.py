...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        #         self.filenames = '/content/workspace/dataset/car.png' #f.readlines()
    ...
    def __getitem__(self, idx):
        # sample_path = self.filenames
        focal = 910 #float(sample_path.split()[2])
        ...
        if self.mode == 'train':
            ...
            image_path = data_path # os.path.join(data_path, "./" + sample_path.split()[0])
            image = Image.open(image_path)
            image = np.asarray(image.resize((640,480)), dtype=np.float32) / 255.0
            if self.mode == 'online_eval':
            ...
            if False : #self.args.do_kb_crop is True:
