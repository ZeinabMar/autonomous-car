...
def test_kitti_3d(dataset_test, test_split, net, rpn_conf, results_path, test_path, use_log=True):
    ...
    imlist = "/content/D4LCN/data_test/image_2.png" #test_path + dataset_test #list_files(os.path.join(test_path, dataset_test, test_split, 'image_2', ''), '*.png')
    ...
    if True : #rpn_conf.depth_channel == 3:
    ...
    # read in calib
    p2 = read_kitti_cal("/content/D4LCN/data_test/calib.txt")  # 3d to 2d #os.path.join(test_path, dataset_test, test_split, 'calib', name + '.txt')
    ...
    for boxind in range(0, min(rpn_conf.nms_topN_post, aboxes.shape[0])):
        ...
        if score >= 0.001: #0.75
            ...
            y3d += h3d/2
            print(cls, alpha, x1, y1, x2, y2, h3d, w3d, l3d, x3d, y3d, z3d, ry3d, score)
